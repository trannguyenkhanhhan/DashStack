...
const Sidebar = ({ isSidebarOpen,activeItem,onClick }) => {
  const mainNavItems = [
        ...
        <nav className="flex-1 px-2 py-4 space-y-6 overflow-y-auto no-scrollbar">
            <ul className="space-y-1">
                {mainNavItems.map((item) => (
                    <NavItem key={item.name} isActive={item.name === activeItem} onClick={onClick} item={item} isSidebarOpen={isSidebarOpen} />
                ))}
            </ul>
            <div>
