...
function ProductCard({ products, onReload }) {
  ...
  const toggleFavorite = async (item) => {
    ...
    await ApiService.callApiAsync("productsales", "PUT", updatedData, item._id);
    onReload()
  };
      ...
      {products.map((item, index) => (
        <div key={index} className="flex flex-col items-center">
          <div className="bg-[#273142] rounded-2xl shadow-md w-full max-w-md overflow-hidden">
            ...
            />
            <h2 className="mt-3 text-lg font-semibold ml-3 text-white">
              {item.product_name}
            </h2>
            ...
            <div className="flex justify-between gap-1 text-yellow-400 text-sm m-3">
              <div>
                ...
                <span className="text-gray-500 text-mg px-2">
                  ({item.rate_count})
                </span>
              </div>
